...
async def main():
    ...
    answers = await (
        AG(
            atype=Answer,
            llm=ollama_llm,  # Replace with LLM Provider / API key in .env
            instructions=(
                "Provide an Answer for the following input text only if "
                "it contains an appropriate question that do not contain "
