...
# Suppress Kafka IPv6 connection error logs
...
def create_pydantic_from_json_schema(
    ...
    Args:
        ...
        model_name: Optional override for the model class name. If not provided,
            uses the "title" field from the schema, or "DynamicModel" as fallback.

    Returns:
        A dynamically created Pydantic model class, or ``None`` on error.
    """
    try:
        # Use provided model_name, or fall back to schema title, or "DynamicModel"
        final_model_name: str = (
            model_name
            if model_name is not None
            else json_schema.get("title", "DynamicModel")
        )
        ...
        return create_model(final_model_name, **field_definitions)
...
def get_atype_from_registry(
    ...
    Automatically detects whether the schema is AVRO or JSON format and uses
    ...
    try:
        ...
        # Detect schema format: AVRO schemas have "type": "record", JSON schemas have "type": "object"
        schema_type = parsed_schema.get("type")

        if schema_type == "record":
            # AVRO schema - use avro_schema_to_pydantic
            from agentics.core.streaming.avro_utils import avro_schema_to_pydantic

            atype = avro_schema_to_pydantic(parsed_schema, model_name=atype_name)
        else:
            # JSON schema - use create_pydantic_from_json_schema
            atype = create_pydantic_from_json_schema(
                parsed_schema, model_name=atype_name
            )
...
def register_schema(
    ...
    """
    Register a Pydantic model's schema in the schema registry.
    ...
    Args:
        ...
        schema_type: Schema format - "AVRO" (default) or "JSON".
            AVRO is recommended for Flink SQL compatibility.
    ...
    try:
        if schema_type == "AVRO":
            ...
            from agentics.core.streaming.avro_utils import pydantic_to_avro_schema

            schema_dict = pydantic_to_avro_schema(atype)
        else:
            # JSON schema
            schema_dict = atype.model_json_schema()

        schema_data = {"schemaType": schema_type, "schema": json.dumps(schema_dict)}
...
def register_atype_schema(
    ...
    """
    Register a Pydantic model's schema in the schema registry using the
    ...
    Args:
        ...
        compatibility: Compatibility mode (``"BACKWARD"``, ``"FORWARD"``,
            ``"FULL"``, ``"NONE"``).
        schema_type: Schema format - "AVRO" (default, recommended for Flink SQL) or "JSON".
...
def create_kafka_topic(
    ...
    Args:
        ...
        kafka_server: Kafka bootstrap server address. If None, reads from
    ...
    # Normalize bootstrap server to avoid IPv6 connection issues
    ...
    try:
        admin_client = KafkaAdminClient(
            bootstrap_servers=bootstrap, client_id="agstream-topic-creator"
        )
        try:
            topic = NewTopic(
                name=topic_name,
...
def kafka_topic_exists(
    ...
    try:
        admin_client = KafkaAdminClient(
            bootstrap_servers=bootstrap, client_id="agstream-topic-checker"
        )
        try:
...
def get_topic_partition_count(
    ...
    try:
        admin_client = KafkaAdminClient(
            bootstrap_servers=bootstrap, client_id="agstream-partition-checker"
        )
        try:
            # Get topic metadata
...
def increase_topic_partitions(
    ...
    try:
        admin_client = KafkaAdminClient(
            bootstrap_servers=bootstrap, client_id="agstream-partition-increaser"
        )
        try:
