...
class Attribute(BaseModel):
    ...
    relation_description: Optional[str] = Field(
        None, description="relation description"
    )
...
class Attributes(BaseModel):
    attributes: Optional[list[Attribute]] = Field(
        None, description="list of Attribute states"
    )
...
async def main(args):
    ...
    mimic_states = load_states(
        ...
        False,
        False,
        lambda x: x["relation_name"].lower() == mimic_relation,
    )
    ...
    if args.matching_type == "1toN":
        omop_states = load_states(
            ...
            True,
            True,
            lambda x: x["relation_name"].lower() == omop_relation,
        )
    ...
    mimic_omop = await mimic_omop.self_transduction(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--matching-type", choices=["1to1", "1toN", "Nto1", "NtoM"], default="1to1"
    )
    ...
    result_name = (
        f"{model_id_to_name[MODEL_ID]}__{args.data_split}__{args.matching_type}"
    )
